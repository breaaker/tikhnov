...
# 定义网格
class cube:
    def __init__(self, l, r, b, t, value):
        self.l = l
        self.r = r
        self.b = b
        self.t = t
...
# 给定边界生成网格
class grid:
    def __init__(self, l, r, b, t, values, m, n):
        self.cubes = []
        self.values = values
        dx = (r - l) / m
        dy = (t - b) / n
        for i in range(m):
            for j in range(n):
                self.cubes.append(cube(l + i * dx, l + (i+1) * dx, b + j * dy, b + (j+1) * dy, values[i][j]))
...
parser = argparse.ArgumentParser(description='CT image reconstruction parameters')
parser.add_argument('--num_x', type=int, default=40, help='Number of grid points in x direction')
parser.add_argument('--num_y', type=int, default=40, help='Number of grid points in y direction')
parser.add_argument('--num_lines', type=int, default=20, help='Number of projection lines')
parser.add_argument('--lambda_1', type=float, default=1e-2, help='Regularization parameter 1')
parser.add_argument('--lambda_2', type=float, default=1e-4, help='Regularization parameter 2')
...
values = draw.draw_value(l, r, b, t, num_x, num_y)
values = values[:, ::-1]

from scipy.signal import convolve2d
smooth_num = 5
kernal = np.ones((smooth_num, smooth_num)) / smooth_num**2
values = convolve2d(values, kernal, mode='same')

grid_ = grid(l, r, b, t, values, num_x, num_y)

# 可在此处修改生成弦的方式 ########################################
lines = generate_line(0, -1, 0, 180, num_lines)
lines += generate_line(1, 0.5, 90, 270, num_lines)
lines += generate_line(1, -0.5, 90, 270, num_lines)
...
plt.savefig("reconstruct.png", dpi=300)
plt.show()
plt.close()

RMSE = np.sqrt(np.sum((ori - guess)**2) / (num_x * num_y))
print("RMSE:", RMSE)
PSNR = 20 * np.log10(np.max(guess)/RMSE)
print("PSNR:", PSNR)
mu_ori, sigma_ori = np.mean(ori), np.std(ori)
mu_re, sigma_re = np.mean(guess), np.std(guess)
covi = np.sum((ori - mu_ori) * (guess - mu_re)) / (num_x * num_y)
corr = covi / (sigma_ori * sigma_re)

C_1, C_2 = 0.01, 0.03
SSIM = (2 * mu_ori * mu_re + C_1) * (2 * covi + C_2) / ((mu_ori**2 + mu_re**2 + C_1) * (sigma_ori**2 + sigma_re**2 + C_2))
print("SSIM:", SSIM)

with open("result.txt", "a") as f:
    f.write(str(lambda_1) + "\t" + str(lambda_2) + "\t" + str(RMSE) + "\t" + str(PSNR) + "\t" + str(SSIM) + "\n")
